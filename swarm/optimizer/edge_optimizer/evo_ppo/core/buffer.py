...
class Buffer():
	...
	def __init__(self, capacity, buffer_gpu=False):
		...
		self.s = []; self.ns = []; self.a = []; self.r = []; self.e = []; self.done = []
	...
	def add(self, trajectory):
		...
		for exp in trajectory:
			...
			self.e.append(torch.Tensor(exp[4]))
			self.done.append(torch.Tensor(exp[5]))

		#Trim to make the buffer size < capacity
		while self.__len__() > self.capacity:
			self.s.pop(0); self.ns.pop(0); self.a.pop(0); self.r.pop(0); self.e.pop(0); self.done.pop(0)
	...
	def sample(self, batch_size):
		"""Sample a batch of experiences from memory with uniform probability
			   ...
			   Returns:
				   Experience (tuple): A tuple of (state, next_state, action, shaped_reward, edge_index, done) each as a numpy array with shape (batch_size, :)
		   """
